...
export default class EventUtils {
  ...
  createAnEvent(dateTime, duration, eventOptions) {
    ...
    if (eventOptions.end) {
      if (typeof eventOptions.end === "string")
        eventOptions.end = ud.stringToDate(eventOptions.end);
    ...
    if (typeof this._vuecal.onEventCreate === "function") {
      if (!this._vuecal.onEventCreate(event, () => this.deleteAnEvent(event)))
        return;
    }
  ...
  createEventSegments(e, viewStartDate, viewEndDate) {
    ...
    if (!e.repeat) {
      ...
      timestamp = viewStartTimestamp;
        ...
        viewEndTimestamp,
        e.repeat.until
          ? ud.stringToDate(e.repeat.until).getTime()
          : viewEndTimestamp
      );
    }
    ...
    while (timestamp <= end) {
      ...
      const nextMidnight = ud
        .addDays(new Date(timestamp), 1)
        .setHours(0, 0, 0, 0);
      ...
      if (e.repeat) {
        ...
        if (
          ...
          (e.occurrences && e.occurrences[tmpDateFormatted])
        ) {
  ...
  checkCellOverlappingEvents(cellEvents, options) {
    ...
    cellEvents.forEach((e) => {
      ...
      if (!_cellOverlaps[e._eid])
        _cellOverlaps[e._eid] = { overlaps: [], start: e.start, position: 0 };
      ...
      _comparisonArray.forEach((e2) => {
        ...
        const eventsInSameTimeStep = options.overlapsPerTimeStep
          ? ud.datesInSameTimeStep(e.start, e2.start, options.timeStep)
          : 1;
        ...
        if (
          ...
          !e2.allDay &&
          eventIsInRange &&
          eventsInSameTimeStep
        ) {
          _cellOverlaps[e._eid].overlaps.push(e2._eid);
          _cellOverlaps[e._eid].overlaps = [
            ...new Set(_cellOverlaps[e._eid].overlaps),
          ]; // Dedupe, most performant way.

          _cellOverlaps[e2._eid].overlaps.push(e._eid);
          _cellOverlaps[e2._eid].overlaps = [
            ...new Set(_cellOverlaps[e2._eid].overlaps),
          ]; // Dedupe, most performant way.
    ...
    for (const id in _cellOverlaps) {
      ...
      overlapsRow.sort((a, b) =>
        a.start < b.start ? -1 : a.start > b.start ? 1 : a.id > b.id ? -1 : 1
      );
      item.position = overlapsRow.findIndex((e) => e.id === id);

      longestStreak = Math.max(
        this.getOverlapsStreak(item, _cellOverlaps),
        longestStreak
      );
    }
  ...
  eventInRange(event, start, end) {
    ...
    if (event.allDay || !this._vuecal.time) {
      ...
      return (
        endTimestamp >= new Date(start).setHours(0, 0, 0, 0) &&
